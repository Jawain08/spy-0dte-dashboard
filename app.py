...
# Updated imports to include the 0DTE ATM contract fetcher
from data_sources import (
    fetch_intraday_data_alpaca, 
    fetch_news_alpaca, 
...
# ---------------------------------------------------------------------------
# 1. DATA INGESTION (YFinance Fallback)
# ---------------------------------------------------------------------------
...
def build_chart(df: pd.DataFrame, show_smas: bool, shade_chop: bool,
    ...
    fig.add_trace(
        go.Scatter(
            x=calls.index, y=calls["Low"] * 0.9985,
            name="Ideal CALL", mode="markers",
    ...
    fig.add_trace(
        go.Scatter(
            x=puts.index, y=puts["High"] * 1.0015,
            name="Ideal PUT", mode="markers",
    ...
    if orh is not None and orl is not None:
        fig.add_hline(y=orh, line=dict(color="#5DCAA5", dash="dash", width=1),
                      annotation_text="OR high", annotation_font_size=10, row=1, col=1)
        fig.add_hline(y=orl, line=dict(color="#F0997B", dash="dash", width=1),
                      annotation_text="OR low", annotation_font_size=10, row=1, col=1)
    ...
    if levels:
        ...
        for name, value in levels.items():
            ...
            fig.add_hline(y=value, line=styles.get(name, dict(width=1)),
                          annotation_text=name, annotation_font_size=10, row=1, col=1)
    ...
    if shade_chop:
        for session_date in sorted(set(df.index.date)):
            ...
            fig.add_vrect(
                ...
                line_width=0, annotation_text="chop zone",
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        data_source = st.radio(
            ...
            index=0,
            help="IEX is real-time but covers ~2% of tape volume.",
        )
    ...
    if raw.empty:
        st.error("No intraday data returned. Check market hours and API keys.")
        st.stop()

    raw = resample_bars(raw, tf_minutes)
    min_bars = slow_sma + rsi_period
    if len(raw) < min_bars:
        st.warning(f"Only {len(raw)} bars at {tf_minutes}-min — indicators need ~{min_bars} to warm up.")
    ...
    # -----------------------------------------------------------------------
    # Current-bar bias banner and Actionable 0DTE Contract Display
    # -----------------------------------------------------------------------
    if last["Call_Signal"]:
        st.success("🟢 **IDEAL CALL TRIGGERED** — VWAP breakout with rising RSI.")
        
        with st.spinner("Fetching optimal 0DTE Call contract..."):
            atm_call = fetch_atm_0dte_contract(underlying_price=last["Close"], option_type="C")
            
        if "error" not in atm_call or not atm_call["error"]:
            cc1, cc2, cc3 = st.columns(3)
            cc1.metric("Target Strike (0.50 Δ)", f"SPY {atm_call['strike']}C")
            cc2.metric("Ask Price (Premium)", f"${atm_call['ask']:.2f}")
            
            if atm_call["is_liquid"]:
                cc3.metric("Bid/Ask Spread", f"${atm_call['spread']:.2f}", "Liquid ✅", delta_color="off")
            else:
                cc3.metric("Bid/Ask Spread", f"${atm_call['spread']:.2f}", "Illiquid ❌", delta_color="inverse")
        else:
            st.warning(f"Could not load live option data: {atm_call.get('error')}")

    elif last["Put_Signal"]:
        st.error("🔴 **IDEAL PUT TRIGGERED** — VWAP breakdown with falling RSI.")
        
        with st.spinner("Fetching optimal 0DTE Put contract..."):
            atm_put = fetch_atm_0dte_contract(underlying_price=last["Close"], option_type="P")
            
        if "error" not in atm_put or not atm_put["error"]:
            cc1, cc2, cc3 = st.columns(3)
            cc1.metric("Target Strike (0.50 Δ)", f"SPY {atm_put['strike']}P")
            cc2.metric("Ask Price (Premium)", f"${atm_put['ask']:.2f}")
            
            if atm_put["is_liquid"]:
                cc3.metric("Bid/Ask Spread", f"${atm_put['spread']:.2f}", "Liquid ✅", delta_color="off")
            else:
                cc3.metric("Bid/Ask Spread", f"${atm_put['spread']:.2f}", "Illiquid ❌", delta_color="inverse")
        else:
            st.warning(f"Could not load live option data: {atm_put.get('error')}")

    elif last.get("In_Chop_Zone", False):
        st.info("⏸️ Inside the midday chop zone — signals suppressed until 1:30 PM ET.")

    # Scheduled-event caution
    now_et = pd.Timestamp.now(tz=EASTERN)
    if now_et.weekday() < 5:
        for event_t in (dt.time(8, 30), dt.time(10, 0), dt.time(14, 0)):
            ...
            if abs((now_et - event_dt).total_seconds()) <= 15 * 60:
                st.warning(f"⚠️ Within 15 min of the {event_t.strftime('%-I:%M %p')} ET window. Technicals may be unreliable.")
                break
